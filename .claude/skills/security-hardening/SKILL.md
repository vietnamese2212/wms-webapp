...
| **Đăng nhập / phiên** | Bộ đếm ở DB (RPC `auth_throttle`), không ở RAM lambda. Thông báo lỗi KHÔNG tiết lộ tài khoản có tồn tại. Khoá phải có đường MỞ trong app (`DELETE /employees/:id/lock`, quyền `user_admin.unlock`) — siết mà không có van = ngõ cụt. **Đừng hạ lại express-rate-limit /login dưới 200**: MemoryStore không mở khoá được, QA 42+43+45 (55 lần sai) và NAT kho đều trip (đo 03/09) | QA 42 · QA 43 mục [3] |
...
| **Thư viện mới** (`npm i`) | `node scripts/qa/44-npm-audit.mjs` — high/critical GỐC không được tăng so `audit-baseline.json` (chỉ đếm gói có advisory, không đếm gói bị lây theo chuỗi — npm 10 runner và npm 11 dev đếm khác nhau, CI đỏ 6 lần 03/09). Vá được thì `npm audit fix` rồi `--update-baseline`. Không đo được → skip có `::warning`, không đỏ | QA 44 (CI job static) |
