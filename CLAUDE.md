...
| `outbound` | Xuất kho | Xuất kho | view, **prepare**=Chuẩn bị hàng (board soạn hàng, read-only — tách khỏi view, không phải ai xem Xuất kho cũng vào được), create, **quick_export**=Tạo & Xuất luôn (đơn tạo tay TOÀN hàng không tem — mã no-QR/kho QTY: 1 request tạo+Bắt đầu(biển số bắt buộc)+ghi nhận SL+Hoàn thành+trừ tồn; `POST /outbound/quick-export`), **import**=Upload Excel KH xuất, **edit**=sửa đơn/xe/đổi ngày/tạm dừng-tiếp tục, assign, unassign, start (kho QTY/NONE không cần assign trước — BE tự gán người bấm Bắt đầu), unstart, **scan**=quét QR + Lưu thủ công no-QR (+ nút "Lưu tất cả theo KH" bulk trên trang chuyến), **complete**=Hoàn thành chuyến (patchGDO status=COMPLETED, controller kiểm riêng — KHÔNG đi ké edit) + xác nhận nhặt lẻ, uncomplete, cancel, **weigh_waive**=Duyệt bỏ qua cổng/cân (chốt LẠI 01/08 — **2 RULE ĐỘC LẬP per kho**, 2 checkbox form Kho WMS Settings: **Rule 1 `Warehouse.require_gate_on_start`** = Bắt đầu phải gắn ĐĂNG KÝ CỔNG hợp lệ — đúng kho, chiều XUẤT, đã VÀO, biển khớp; **xe ĐÃ RA vẫn hợp lệ** (nhập bù/bốc thêm đơn); kho bật rule 1 = picker **KHÔNG còn ô nhập biển vãng lai** (xe không đăng ký = vi phạm rule 1), tick đặc biệt chỉ còn nghĩa "xe đã ra / bốc nhiều đơn" → 422 `GATE_REQUIRED`. **Rule 2 `require_weigh_on_start`** = biển khớp phiếu cân CHƯA hoàn thành hôm nay → 422 `WEIGH_REQUIRED`. Bật rule nào chấp hành rule đó, bật cả 2 = đủ cả 2. **MIỄN TRỪ DUY NHẤT = duyệt TRƯỚC trên chuyến** `POST/DELETE /outbound/:gdoId/weigh-waive` (nút "Bỏ qua cổng/cân" ở detail — phủ cả 2 rule, ghi vết ai/lúc/lý do; chuyến đã duyệt → biển số TÙY CHỌN cho giao lẻ/xe máy/NV nhận) — user chốt "bắt đầu và chọn là rủi ro" nên **KHÔNG còn lựa chọn bypass nào lúc bấm** (đã BỎ: toggle Giao lẻ tự khai + cột small_delivery_* + cờ body weigh_waive + nút duyệt trong dialog — 20260801c drop). Qua rule cân = phiếu tự gắn chuyến `matched_by='auto-start'` để đối chiếu KL. 2 đường "Xuất luôn" chỉ gate rule CÂN — không có UI chọn cổng, kho QTY/NONE) |
