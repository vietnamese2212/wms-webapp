...
| `slotting` | Tối ưu vị trí (Slotting) | Tối ưu vị trí (menu Kho WMS) — phân tích ABC theo lượt nhặt + kế hoạch sắp xếp kho (tiến độ suy sống từ vị trí pallet) | view, **plan**=tạo/xóa kế hoạch sắp xếp (preview+create+delete), **complete**=hoàn thành/hủy/mở lại kế hoạch. Hạng nhặt khu (`WarehouseZone.pick_rank`) sửa trong tab Khu vực = quyền `wms_settings.manage_zone`; công nhân thực hiện chuyển pallet dùng `inventory.move_location` (trang Tồn kho) — kế hoạch tự tick |
