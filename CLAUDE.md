...
| `forklift` | Xe nâng | Xe nâng (menu Kho WMS) — check list an toàn HÀNG NGÀY + đồng hồ giờ vận hành, 3 tab: Check list ngày (board — xe CHƯA check xếp LÊN ĐẦU, có cột Người check/Lúc) · Báo cáo vận hành (giờ chạy = hiệu số đồng hồ 2 lần ghi liên tiếp, RPC `forklift_report` trả cả `checked_at`; bảng chi tiết ngày = lịch sử check kèm ai/lúc nào) · Cài đặt (filter theo Kho). Mỗi xe mỗi ngày 1 bản ghi (unique, upsert đè — KHÔNG gửi `id` trong upsert kẻo đè khóa chính); xe NGHỈ = status IDLE không cần số; số đồng hồ chỉ tăng (BE kẹp 2 chiều 422). **Hạng mục check list THEO KHO** (`warehouse_id` null = dùng chung mọi kho): check list 1 xe = bộ chung + bộ riêng kho xe đó; sửa/xóa hạng mục DÙNG CHUNG = chỉ user full scope kho (như khung giờ cargo ALL), hạng mục riêng kho = user có kho đó | view, **check**=ghi/sửa/xóa check list ngày, **manage_vehicle**=danh mục Xe (tab Cài đặt), **manage_item**=hạng mục check list (tab Cài đặt) |
