...
- **Scope Kho + Loại hàng (`Employee.warehouse_ids` + `Employee.allowed_categories`, 02/07):** taxonomy Loại hàng = LookupValue `warehouse_type` (Thành phẩm/POSM/Raw/Giấy/Thùng — giá trị cũ NVL/Bao bì đã dọn bằng migration `20260702_normalize_allowed_categories`, NVL→Raw+Giấy+Thùng, Bao bì→Giấy+Thùng). **FE**: mọi filter/form nghiệp vụ lấy option Loại kho qua `useScopedWhTypes()` và Kho qua `useScopedWarehouses()` (`frontend/src/hooks/useUserScope.ts`) — trang admin (WMSSettings/UserManagement/TMSSettings) cố ý dùng hook gốc đầy đủ. **BE**: util `backend/src/utils/categoryScope.ts` (`scopeCategoriesOf`/`categoryAllowed`) — guard write ở inbound `createOrder`, outbound `createGDO`/`updateGDO`, tms `createOrder`/`updateOrder`, pallet-prints `logPrints`; cắt list ở `listGDOs` + `listPrints` (dòng không khai loại/kho vẫn hiện); pallet-ops guard theo **kho thật của entry** (`guardEntryWh`), không tin warehouse_id từ body. Khi thêm form/filter mới có chọn Kho/Loại kho → BẮT BUỘC dùng 2 hook scoped + guard BE tương ứng.
