...
| `forklift` | Xe nâng | Xe nâng (menu Kho WMS) — check list an toàn HÀNG NGÀY + đồng hồ giờ vận hành, 3 tab: Check list ngày (board xe chưa check) · Báo cáo vận hành (giờ chạy = hiệu số đồng hồ 2 lần ghi liên tiếp, RPC `forklift_report`) · Cài đặt. Mỗi xe mỗi ngày 1 bản ghi (unique, upsert đè — KHÔNG gửi `id` trong upsert kẻo đè khóa chính); xe NGHỈ = status IDLE không cần số; số đồng hồ chỉ tăng (BE kẹp 2 chiều 422) | view, **check**=ghi/sửa/xóa check list ngày, **manage_vehicle**=danh mục Xe (tab Cài đặt), **manage_item**=hạng mục check list (tab Cài đặt) |
