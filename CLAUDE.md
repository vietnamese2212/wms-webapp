...
| `outbound` | Xuất kho | Xuất kho | view, **prepare**=Chuẩn bị hàng (board soạn hàng, read-only — tách khỏi view, không phải ai xem Xuất kho cũng vào được), create, **quick_export**=Tạo & Xuất luôn (đơn tạo tay TOÀN hàng không tem — mã no-QR/kho QTY: 1 request tạo+Bắt đầu(biển số bắt buộc)+ghi nhận SL+Hoàn thành+trừ tồn; `POST /outbound/quick-export`), **import**=Upload Excel KH xuất, **edit**=sửa đơn/xe/đổi ngày/tạm dừng-tiếp tục, assign, unassign, start (kho QTY/NONE không cần assign trước — BE tự gán người bấm Bắt đầu), unstart, **scan**=quét QR + Lưu thủ công no-QR (+ nút "Lưu tất cả theo KH" bulk trên trang chuyến), **complete**=Hoàn thành chuyến (patchGDO status=COMPLETED, controller kiểm riêng — KHÔNG đi ké edit) + xác nhận nhặt lẻ, uncomplete, cancel, **weigh_waive**=Duyệt bỏ qua cổng/cân (01/08 — kho bật cờ `Warehouse.require_weigh_on_start` = quy trình chặt "đăng ký cổng → vào cổng → cân bì → bốc hàng": Bắt đầu đòi (1) gắn ĐĂNG KÝ CỔNG hợp lệ — đúng kho, chiều XUẤT, đã VÀO, biển khớp; **xe ĐÃ RA vẫn hợp lệ** (nhập bù muộn/bốc thêm — user chốt), chỉ khóa biển VÃNG LAI không đăng ký → 422 `GATE_REQUIRED`; và (2) biển khớp phiếu cân CHƯA hoàn thành hôm nay → 422 `WEIGH_REQUIRED`. Xe không cân/không đăng ký được → người có quyền này duyệt trên chuyến `POST/DELETE /outbound/:gdoId/weigh-waive` hoặc cờ body `weigh_waive` ngay lúc bấm — 1 lần duyệt phủ CẢ cổng lẫn cân, ghi vết ai/lúc nào/lý do. **GIAO LẺ (xe máy/nhân viên nhận — user chốt 01/08): KHÔNG cần quyền** — dialog Bắt đầu ở kho bật cờ mở đầu bằng toggle **Loại chuyến: "Xe tải (qua cổng-cân)" / "Giao lẻ — xe máy/NV nhận"** (1 lựa chọn duy nhất — KHÔNG dùng checkbox rời vì trùng hình với tick "Trường hợp đặc biệt" trong picker, user bắt lỗi); chọn Giao lẻ = ẩn picker chuyến cổng, biển số TÙY CHỌN (BE nới guard khi small_delivery), miễn cả cổng lẫn cân, ghi vết `small_delivery_at/by` (TÁCH cột khỏi weigh_waived_* để báo cáo phân biệt tự-khai vs được-duyệt) + badge trên chuyến để quản lý đối soát; gỡ bắt đầu = xóa vết (khai theo LƯỢT). Qua cổng cân = phiếu tự gắn chuyến `matched_by='auto-start'` để đối chiếu KL. 2 đường "Xuất luôn" chỉ gate CÂN — không có UI chọn cổng, kho QTY/NONE) |
