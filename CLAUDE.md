...
- **Scope Kho + Loại hàng (`Employee.warehouse_ids` + `Employee.allowed_categories`, 02/07) — RULE user chốt: "đã phân quyền Kho + Loại kho nào thì CHỈ thấy dữ liệu của Kho + Loại đó".** Taxonomy Loại hàng = LookupValue `warehouse_type` (Thành phẩm/POSM/Raw/Giấy/Thùng — giá trị cũ NVL/Bao bì đã dọn bằng migration `20260702_normalize_allowed_categories`, NVL→Raw+Giấy+Thùng, Bao bì→Giấy+Thùng). **FE**: mọi filter/form nghiệp vụ lấy option Loại kho qua `useScopedWhTypes()` và Kho qua `useScopedWarehouses()` (`frontend/src/hooks/useUserScope.ts`) — trang admin (WMSSettings/UserManagement/TMSSettings) cố ý dùng hook gốc đầy đủ. **BE**: util `backend/src/utils/categoryScope.ts` (`scopeCategoriesOf`/`categoryAllowed`) — (a) guard write 403: inbound `createOrder`, outbound `createGDO`/`updateGDO`, tms `createOrder`/`updateOrder`, gate create/update, pallet-prints `logPrints` (kèm kho từng tem); (b) cắt LIST theo kho+loại: `listGDOs`, `listPrints`, loose-picking, gate list, tms orders (2 nhánh + chặn `warehouse_id` ngoài scope), `listLocations` (kể cả không truyền warehouse_id) + `listSubGroups`, `stocktakeEntries` (validate `location_ids` từ client), inventory `listFacets` (intersect JWT), `listMaterials`, scan-log (RPC `p_allowed_categories` — migration `20260702_scanlog_category_scope`, controller có fallback khi chưa apply); pallet-ops guard theo **kho thật của entry** (`guardEntryWh`), không tin warehouse_id từ body. Quy ước: mọi cắt đều **null-inclusive** (bản ghi không khai loại vẫn hiện). Khi thêm form/filter/list mới có Kho/Loại kho → BẮT BUỘC dùng 2 hook scoped + guard/cắt BE tương ứng.
