...
| `fill` | Fill hàng (nhặt lẻ) | Fill hàng (menu Kho WMS) — 3 tab: **Đề xuất** (per mã: Cần nhặt lẻ của NGÀY XUẤT / Đang có ở vị trí nhặt lẻ / Đang có lệnh / Thiếu / pallet FEFO đề xuất hạ) · **Lệnh fill** (mỗi lệnh = 1 PALLET phải hạ, gán người, quét thực hiện) · **Kết quả** (tỷ lệ hoàn thành theo NGƯỜI). Nền dữ liệu = cờ **`Location.is_pick_face`** ("vị trí nhặt lẻ" — chỗ với tay lấy hàng được), khai HÀNG LOẠT ở trang Vị trí kho (nút "Vị trí nhặt lẻ" → route `bulk-flag` sẵn có, áp theo BỘ LỌC nên không nhồi id qua URL) hoặc tick từng vị trí trong form sửa; mặc định `false` ⇒ kho chưa khai thì trang cảnh báo rõ thay vì báo thiếu oan. Công thức "cần" LẤY NGUYÊN của `loose_picking_page` (một nguồn sự thật). RPC: `fill_demand` (cần/có/thiếu + pallet FEFO + đích gợi ý, 1 lời gọi) · `fill_tasks_page` (dòng + 4 ô band) · `fill_report`. **Quét thực hiện** = `POST /wms/fill/scan`: pallet phải ĐANG Ở đúng vị trí nguồn → RPC `move_pallets_to_location` (khóa sức chứa) → mới đánh DONE; đích đầy = 400 `LOCATION_FULL` và lệnh **VẪN TREO** (đổi đích rồi quét lại — không tạo ngõ cụt); lệnh của người khác = 409 `NOT_YOUR_TASK`, ai có `assign` bấm "Nhận lệnh này" (`take_over`) mới chuyển — không cướp việc âm thầm. Chống đua: unique index `uq_filltask_pending_entry` (1 pallet = 1 lệnh treo), người thua nhận `skipped` có lý do. Gói QA 18-fill-replenish + 3 bất biến gói 00 + ratchet `location_write_without_move_rpc` gác | view, **plan**=ra lệnh / hủy / đổi vị trí đích, **assign**=gán lệnh cho người khác, **execute**=quét thực hiện (ghi `location_id` nhưng CHẶN HẸP: đúng pallet của lệnh + đúng đích của lệnh — cùng tiền lệ `leftover_location_id` bên Xuất; đổi vị trí pallet bất kỳ ngoài lệnh vẫn phải `inventory.move_location`) |
