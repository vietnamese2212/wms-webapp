...
**Verify E2E sống (API thật staging, dọn sạch 0 sót) — 23/23 PASS:**
- API (12): A1 INSERT(2) · A2 NO-OP(2)+id giữ+không bump updated_at · A3 UPDATE(1) id+created_at giữ qty=250 · A4 header guard 400 · B od_refs mọi item đủ field + Σqty_base=cartons_ordered · B khvc_lines persist ACTIVE + re-upload id giữ · B list API materialized/do_ready.
