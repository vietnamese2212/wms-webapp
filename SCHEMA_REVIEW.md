...
| 2026-07-01 | — | `InventoryEntry` — partial UNIQUE index `uq_inventory_active_pallet_code (pallet_code) WHERE status IN ('IN_STOCK','PARTIAL','QUARANTINE','LOOSE_PICKING')`. Chống tạo pallet TRÙNG khi quét ĐỒNG THỜI (race read-then-insert ở scanQR/scanManual — nhiều request cùng đọc "chưa có" rồi cùng INSERT → nhiều entry cùng pallet_code). Handler đã bắt 23505 → 409 "đã tồn"; index khiến 23505 thực sự bắn khi đua. Migration `20260701_inventory_active_pallet_unique.sql` (ĐÃ apply qua pg CONCURRENTLY). Verify: 8 quét cùng mã đồng thời → 1 OK + 7×409 (trước fix: 8 pallet ảo). Phát hiện trong test vận hành Mốc 3. |
