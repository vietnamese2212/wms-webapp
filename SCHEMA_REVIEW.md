...
| 2026-07-18 | — | **Slotting v2 — user chỉnh rule sau review đợt 1**: `WarehouseZone.slot_group` + `flow_type` (SAME_END/FLOW_THROUGH) + `Material.slot_group` (nhóm riêng/khu đặc biệt, vd SCA lạnh — khu có nhóm chỉ nhận mã cùng nhóm, mã có nhóm bị kéo VỀ khu nhóm ưu tiên cao nhất, hàng lạ trong khu riêng CHỈ cảnh báo) + `SlottingPlan.level/principle`. **DROP + tạo lại `SlottingPlanLine`** (bảng cũ rỗng): dòng GOM theo (mã + date_key) — material_id, n_pallets, entry_ids jsonb (suy tiến độ x/N sống), flow_note. RPC `slotting_stats` REPLACE cùng chữ ký (+ slot_group/flow_type). Mức độ Easy/Normal/Hard + nguyên tắc FIFO/FEFO/LIFO = FILTER trên trang (không cột trên Warehouse). Migration `20260718_slotting_v2.sql` — chạy SAU 20260717_slotting.sql (CHỜ apply staging; production khi merge chạy 2 file tuần tự). |
