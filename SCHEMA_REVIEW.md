...
| 2026-06-15 | — | HR Lịch làm việc & Chấm công (nền tảng): `Skill` (vị trí/kỹ năng theo `warehouse_id`+`department_id`, `shift_tag`), `EmployeeSkill` (NV×skill + `priority` 1=sở trường chính), `LeaveRequest` (nghỉ phép), `WorkAssignmentSheet`/`WorkAssignmentDemand`/`WorkAssignment` (phiếu phân công 1 ngày/1 kho/1 phòng), `Attendance` (NV tự khai: kind CA1/CA2/CA3/HC/LEAVE + ot_hours + early_leave_hours, unique emp+date). `Department.requires_scheduling`. Mọi bảng scope theo `warehouse_id`. Migration `20260615_hr_schedule_attendance.sql`. RLS + anon SELECT cho Realtime. |
