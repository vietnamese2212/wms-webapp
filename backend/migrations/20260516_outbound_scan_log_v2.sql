-- get_outbound_scan_log v2
-- Thay đổi so với v1:
--   - p_warehouse_id  → p_warehouse_ids  (comma-separated, multi-select)
--   - Thêm filter: p_material_category, p_pallet_code, p_machine_codes, p_cycles
--   - Thêm cột trả về: material_category, shelf_life_days
--   - Tăng default limit từ 200 → 500
-- get_scan_log_facets: distinct machine_code và cycle (theo material_category)
...
CREATE OR REPLACE FUNCTION get_outbound_scan_log(
  ...
  p_warehouse_ids     text    DEFAULT NULL,   -- comma-separated warehouse IDs
  ...
  p_machine_codes     text    DEFAULT NULL,   -- comma-separated, exact match
  p_cycles            text    DEFAULT NULL,   -- comma-separated, exact match
...
-- Facets: distinct machine_code và cycle theo material_category
CREATE OR REPLACE FUNCTION get_scan_log_facets(
  p_material_category text DEFAULT NULL
)
RETURNS TABLE (
  machines text[],
