-- FIX lệch giờ biểu đồ "Nhịp độ theo giờ" (phát hiện 17/07 khi xem data sống: quét lúc
-- 10:32 VN hiện ở cột 20h). Nguyên nhân gốc: OutboundScanEntry.scanned_at + InventoryEntry.created_at
-- là timestamp WITHOUT time zone lưu giá trị UTC — biểu thức cũ `col AT TIME ZONE 'Asia/Ho_Chi_Minh'`
-- HIỂU NHẦM giá trị naive là giờ VN (lệch +10 khi extract hour ở session UTC). Sửa chuẩn naive-UTC:
--   extract(hour from (col AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Ho_Chi_Minh'))
-- + biên ngày t0/t1 đưa về naive UTC để so sánh không phụ thuộc session timezone.
-- Cùng chữ ký với v3 → CREATE OR REPLACE. Verify: ts '03:31' UTC → giờ 10 (VN) ✓.
...
as $$
with day_range as (
  -- naive UTC (cột giờ trong DB là timestamp-naive chứa UTC)
...
hourly_out as (
  -- naive-UTC → giờ VN: AT TIME ZONE 'UTC' (gắn nhãn UTC) rồi AT TIME ZONE VN (đổi múi)
