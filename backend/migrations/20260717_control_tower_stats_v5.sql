-- Control Tower v5 (user 17/07): thêm cột hiển thị cho các khối —
-- (a) Xe trong cổng: + Loại kho (warehouse_type) + Loại xe (vehicle_type) (Hướng đã có);
-- (b) Chuyến đang soạn: + Loại kho (GDO.warehouse_type) + Loại xe (export_type từ item, distinct).
-- Cùng chữ ký với tz_fix → CREATE OR REPLACE (đè bản 20260717_control_tower_hourly_tz_fix).
