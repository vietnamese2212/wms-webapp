-- Fix lệch định nghĩa sức chứa (test hội tụ 18/07):
-- slotting_stats.loc_used đếm chỗ đã dùng CHỈ stack_layer=1, trong khi RPC move_pallets_to_location
-- (gác cổng khi công nhân chuyển vị trí) đếm MỌI tầng → engine slotting tưởng đích còn chỗ,
-- thực hiện bị 400 LOCATION_FULL oan (7/452 dòng fail trong test). Sửa: bỏ điều kiện stack_layer=1
-- để used_slots = đúng con số mà move RPC sẽ kiểm — 2 phía cùng 1 thước đo.
-- (Thay đổi DUY NHẤT so với 20260718_slotting_locations.sql: xóa "AND ie.stack_layer = 1" trong loc_used.)
