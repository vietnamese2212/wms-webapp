-- FIX DRIFT (22/07/2026): 2 RPC dashboard_stats + control_tower_stats trên STAGING đã được
-- sửa THẲNG trên DB để quy đổi BASE→THÙNG per-mã (chia units_per_carton cho mã có entry_unit)
-- sau đợt BASE UNIT flip (20260719), NHƯNG các file migration cũ (20260704_dashboard_stats,
-- 20260716/17_control_tower_stats_*) VẪN là bản cộng base THÔ → nếu PRODUCTION apply theo file cũ
-- sẽ THỔI TỔNG ×units_per_carton (vd ×48) trên Dashboard + Giám sát vận hành.
--
-- File này CHỤP LẠI đúng định nghĩa RPC đang chạy trên staging (đã verify Dashboard hiển thị
-- 494.711 thùng = số quy đổi ĐÚNG, không phải ~20.8M base thô). Áp file này khi merge production
-- để 2 dashboard hiển thị "thùng quy đổi" khớp trang Tồn kho/Xuất kho (BE fallback JS + FE đều quy đổi).
--
