...
export async function listSlotTemplates(req: Request, res: Response) {
  try {
    const { warehouse_id, vehicle_type_id, direction } = req.query as Record<string, string>
    ...
    let q = (supabase.from('SlotTemplate') as any)
      ...
      .order('vehicle_type_id').order('direction').order('day_of_week').order('time_from')
...
export async function createSlotTemplate(req: Request, res: Response) {
  try {
    const { warehouse_id, vehicle_type_id, direction, cargo_type = 'ALL', days_of_week, time_from, time_to, max_vehicles } = req.body as {
    ...
    if (!warehouse_id || !vehicle_type_id || !direction || !days_of_week?.length || !time_from || !time_to || !max_vehicles)
      return fail(res, 'Thiếu thông tin bắt buộc', 400)
    const now = new Date().toISOString()
    const rows = days_of_week.map(dow => ({
      id: randomUUID(), warehouse_id, vehicle_type_id, direction, cargo_type,
