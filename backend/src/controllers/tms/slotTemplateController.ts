...
export async function listSlotTemplates(req: Request, res: Response) {
  try {
    const { vehicle_type_id, direction } = req.query as Record<string, string>
...
export async function createSlotTemplate(req: Request, res: Response) {
  try {
    const { vehicle_type_id, direction, cargo_type = 'ALL', days_of_week, time_from, time_to, max_vehicles } = req.body as {
    ...
    if (!vehicle_type_id || !direction || !days_of_week?.length || !time_from || !time_to || !max_vehicles)
      return fail(res, 'Thiếu thông tin bắt buộc', 400)
    const now = new Date().toISOString()
    const rows = days_of_week.map(dow => ({
      id: randomUUID(), vehicle_type_id, direction, cargo_type,
