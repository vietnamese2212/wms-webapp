...
export async function listVehicles(req: Request, res: Response) {
  try {
    ...
    let q = (supabase.from('Vehicle') as any)
      .select('*, ncc:TransportCompany(id, code, name), vehicle_type:VehicleType(id, code, name)')
      .order('license_plate')
...
export async function createVehicle(req: Request, res: Response) {
  try {
    ...
    const { data, error } = await (supabase.from('Vehicle') as any)
      ...
      .select('*, ncc:TransportCompany(id, code, name), vehicle_type:VehicleType(id, code, name)').single()
    if (error) return fail(res, error.message)
