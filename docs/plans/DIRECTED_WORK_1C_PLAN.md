...
### 0.10 BẮT BUỘC CHỌN XE NÂNG CHUYỂN KHI BẮT ĐẦU (kho Hướng dẫn) — bổ sung/đổi được sau (user 10/09 vòng 6)
- Cột mới `GroupDeliveryOrder.forklift_driver_ids text[]` (nhiều người; giữ `forklift_driver_id`/`forklift_driver_names` cũ để bundle cũ + báo cáo cũ không vỡ — ghi cả hai: `forklift_driver_id` = người đầu, `names` = tên nối ", ").
- **`startGDO`, kho `work_mode=GUIDED`** (theo resolver 2 tầng, loại của chuyến) **và không phải Xuất luôn**: thiếu `forklift_driver_ids` (rỗng) → **422 `FORKLIFT_REQUIRED`** "Kho này chạy Hướng dẫn — chọn ít nhất một xe nâng chuyển". Cặp nội bộ vẫn bắt (vẫn cần người đưa hàng). Kho THỦ CÔNG: tuỳ chọn như hôm nay. Id phải là nhân sự **thuộc kho** (`getWarehouseEmployees` — route `/fill/employees` đang dùng cho Fill assign; tái dùng, thêm alias `/outbound/employees` gate `outbound.start|edit`), id lạ → 400/404.
- **Bổ sung/đổi sau**: `updateTransport` (Sửa thông tin xe) nhận `forklift_driver_ids[]` (thêm/bớt), giữ luật GUIDED không cho về rỗng khi chuyến còn việc PENDING. Ghi `outbound_events` ("Xe nâng: +Hùng, −Nam").
- FE: ô **Xe nâng chuyển** trong StartDialog thành **chọn nhiều** (`SingleSelect` → multi chip, tìm trên server theo tên/mã NV), dấu * khi kho GUIDED; cùng ô trong EditTransportDialog; thẻ chuyến hiện danh sách tên. Bảng "Cần đưa ra" chip **"Của tôi"** = `user.id ∈ forklift_driver_ids`.
- QA 57: [30] GUIDED thiếu xe nâng → 422 FORKLIFT_REQUIRED; MANUAL → 200 không cần · [31] 2 người → cả hai thấy "Của tôi"; Sửa xe bớt 1 → người đó hết thấy, về rỗng khi còn việc → 422 · [32] id ngoài kho → 400.

### 0.7 HIỂN THỊ CHỈ DẪN (user chốt 10/09: "không văn xuôi, đưa vào table; xong thì gạch; có nút đánh dấu phòng quên")
- Chỉ dẫn cho xe nâng gom **theo VỊ TRÍ** (mỗi tầng là một `Location` riêng, mã đã mang khu·dãy·tầng): một dòng = *`B_TP1_29_T3` · hạ **4 pallet** → Điểm đặt dãy 2*. Mọi cột vị trí in `location_code` nguyên văn — không tách "ô/tầng" (user 10/09); `level_no` chỉ dùng để tính (Cần hạ, thứ tự) (việc vẫn lưu từng pallet trong `wms_tasks`, RPC gom `GROUP BY from_location, level, drop` cho mode LOWER/MOVE; mode SCAN của thủ kho vẫn từng pallet vì quét theo tem). Không ghim tem lên bảng xe nâng — khớp chốt 08/09 "xe hạ không quét".
...
| 3.2 | **Móc vào `startGDO`**: TRƯỚC CAS — kho GUIDED & không Xuất luôn & rỗng `forklift_driver_ids` → 422 `FORKLIFT_REQUIRED` (0.10); sau CAS thành công (nơi đang ghi `dock_location_id`), gọi `planGdoTasks`; lỗi → `recordServerError` + trả `plan_warning` trong payload, **không** 5xx. `unstartGDO`, `patchGDO status=CANCELLED`, `patchGDO status=COMPLETED` (+ đường `manualComplete`/complete khác đặt COMPLETED) → `cancelGdoTasks` (reason theo đường). | QA 57: start kho GUIDED → n việc; unstart → 0 PENDING, có CANCELLED; kho MANUAL → 0 việc; start vẫn 200 khi bản vẽ bị xoá tạm (warning) |
