...
| 4.2 | **Trang mới `pages/wms/DirectedWork.tsx`** route `/wms/directed` (PermissionRoute `directed_work.view`), menu Kho WMS "Lệnh việc", filter Kho qua `useScopedWarehouses` + slice filter mới (`useWmsFilterStore` — **BẮT BUỘC thêm dòng `sweepGlobalScope`**). 3 tab: **Cần hạ** (toàn kho; cột: STT · Chuyến/Biển · Cửa · Mã · Pallet · Ô · Tầng · Khoảng cách (ô × cell_m → m) · Điểm đặt dãy · nút "Đã hạ" [gate `lower`]; dòng đầu nổi bật; badge "xe đang chờ"), **Cần đưa ra bãi** (chip "Của tôi" = `forklift_driver_id === user.id` / theo chuyến; cột: Chuyến · Cửa · Mã · Pallet · Từ ô (đã hạ → điểm đặt dãy) · Tới cửa · STT), **Sắp quét** (chọn chuyến; dòng kế tiếp nổi bật; hiện "đã hạ ✓" / "chờ hạ ⏳"). Hook `useDirectedBoard(wh, mode, gdo?, driver?)` key `['directed-board', …]`; `useLowerTask`, `useReplanGdo` invalidate `['directed-board']`, `['gdo', id]`. Theo skill `table-format` (SummaryBand: việc treo · cần hạ · đã hạ chờ chuyển · chuyến đang chạy). | Playwright 1280/390/360 không tràn; dòng đầu nổi; bấm Đã hạ → dòng đổi trạng thái không F5 |
