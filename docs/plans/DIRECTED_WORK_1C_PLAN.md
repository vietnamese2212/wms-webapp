...
- **CHỐT vòng 3 (user 10/09): bỏ trống = FEFO, không phải "chờ chốt".** Thủ kho có **một bước chốt %Date TRƯỚC khi xuất / nhặt lẻ** cho các dòng còn trống; không chốt thì máy chia FEFO thuần. Không có trạng thái chặn.
- Mỗi dòng đơn mang **một quy tắc date có cấu trúc** (cột jsonb `OutboundItem.date_rule` `{kind, value, source, set_by, set_at}`), 3 dạng: `MIN_PCT` (≥ n %) · `EXACT` (NSX hoặc HSD hoặc lô — `batch_required` sẵn có làm value) · `NEWEST` (LIFO cho dòng này — trung chuyển). Trống = FEFO. `date_required` sẵn có = `MIN_PCT` cũ, đọc tương thích (không đổi cột cũ để bundle cũ vẫn chạy).
- **Nguồn, ghi vào `source`:** `MANUAL` (thủ kho chốt — đường chính) · `SAP` (VL06O có lô/NSX → EXACT tự điền, không ai gõ — **chờ user xác nhận VL06O trung chuyển có cột này không**) · `DEFAULT` (tuỳ chọn, khai ở Cài đặt kho theo loại đơn — vd trung chuyển → NEWEST; NPP để trống = FEFO như user chốt).
...
- Bộ sinh việc chia MỌI dòng: rule trống → FEFO thuần; `MIN_PCT` → lọc `computePctDate ≥ n` rồi FEFO; `EXACT` → đúng NSX/HSD/lô; `NEWEST` → LIFO. QA 57 oracle theo từng dạng + ca "ghi chú CS mà rule trống ⇒ vẫn chia FEFO + `plan_warning`". **Mục 0.6 (ngưỡng `auto_pick_min_pct` toàn kho + "vùng chỉ định") HUỶ** — thay bằng mục này; `pinned_pallets` giữ làm dạng `EXACT` theo tem (tuỳ chọn trong dialog).
