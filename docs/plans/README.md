...
| [TMS_DISPATCH_PLAN.md](TMS_DISPATCH_PLAN.md) | **TMS điều vận**: ZSD02 thay VL06O làm nguồn DO (hai sổ OD/SO, số base = SAP ở dòng có OD · dẫn xuất có cờ ở dòng chưa OD · flow phân loại dòng · công tắc `sap_do_source`), master địa lý/tuyến/dòng xe, bảng cước theo phường × ĐVVT × dòng xe, engine ghép chuyến, đối soát. | Đợt 0 (ZSD02 + master) code 22/09 trên `dev`; đợt 1–4 chờ |
| [DATE_RULE_MASTER_V2_PLAN.md](DATE_RULE_MASTER_V2_PLAN.md) | **Quy định date** (đổi tên từ Chốt %Date) — mức theo cặp **khách × loại hàng** trong bảng `date_rule_master` dùng chung khách/kênh, thêm kiểu **≥ số ngày** (FG02 hạn 45–60 ngày nên % không diễn đạt nổi), mã không hạn dùng tự "không đòi mốc", cửa gác mở rộng theo `date_rule_policy`. | ✅ XONG 11/09 (dev acd0a512→08bca5aa, QA 58 64/64) |
