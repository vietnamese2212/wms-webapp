...
> **Kiểm lại 22/09 chiều (user yêu cầu "test lại cẩn thận"):** `--tier fast` ĐẦY ĐỦ trên staging bắt 2 bất biến gói 00 mà bậc offline trước push không thấy — `erp_so_lines` + `sap_route` **quên bật RLS**, và `storage_location` (Sloc THÔ của SAP, trên cả 2 sổ) bị RPC gác cascade đổi tên Loại kho coi là "cột mang Loại kho chưa cascade". Vá bằng `20260922c_zsd02_rls_and_sloc_exempt.sql` (bật RLS · MIỄN TRỪ 3 cột mã SAP thô khỏi phép kiểm — không cascade vì đổi tên trong app không đổi được mã SAP, lần nạp sau lại ghi FG01). Sau vá: fast 🟢 (bất biến 32/32 · smoke 19/19 · fuzz 97/97 · quyền 14/14) · QA 59 22/22 · `npm test` 91 · tsc BE/FE · oracle SQL: 0 dòng chưa OD lọt sổ OD · 0 OD base 0 · 72/72 dòng chưa OD có `qty_so_base` = SL × hệ số quan sát từ chính sổ OD · band tab Chưa có OD (71 · 9 SO · 8 ship-to · 62,8 pallet · 53,4 tấn) khớp SQL từng số · 360 px không tràn · console 0 lỗi. **Bài học:** migration mới ⇒ phải chạy `--tier fast` có DB SAU khi áp, bậc offline không đo được RLS/độ phủ cascade. **Lỗi thứ 3 (soi UI):** nạp lại đúng lát đã nạp mà bảng kiểm-trước in "Sẽ thêm 3.050 · Sẽ cập nhật 0" trong khi ghi thật là NO-OP — cả ZSD02 lẫn VL06O đều đếm `toInsert = cả file`; nay bước so sánh với sổ (giữ id · bizHash) chạy TRƯỚC kiểm-trước ở cả hai cửa ⇒ ô "Sẽ thêm / Sẽ cập nhật / Không đổi / SAP đã bỏ" là cùng phép so với đường ghi; FE hộp kết luận "không có gì mới" là thông tin xám, không còn hộp đỏ "Không thể nhập". Gói 59 thêm [3a0] (ZSD02 kiểm-trước lần 2 = 0/0/5) + [3b0] (VL06O SL đổi = 0 thêm / 1 cập nhật).
...
> **23/09 chiều — mục 14+15 phần chuyến ĐÃ LÊN dev:** ô **Dòng xe con (mã SAP)** cấp xe ở editor Kế hoạch xuất (cùng khuôn cửa booking: 1 ô cho cả xe, BE đồng bộ mọi dòng + sự kiện `PLAN_VEHICLE_MODEL_CHANGED`; thêm DO vào xe đã có → ép theo xe; upload KH nhận cột tuỳ chọn "Mã xe SAP", sai → 400 `VEHICLE_MODEL_INVALID`, không có cột → GIỮ lựa chọn tay) → dội xuống `GroupDeliveryOrder.vehicle_model_id` (insert · preserve · merge PAUSED · vân tay kế hoạch) → `services/freightEstimate.ts` tính **cước dự tính + tải** ghi `freight_estimated/freight_tariff_id/freight_detail` ngay sau derive (basis PLAN), khi Hoàn thành (basis ACTUAL theo số quét) và qua `POST /tms/freight/recompute {warehouse_id?, date_from, date_to ≤ 62 ngày}` (bảng cước nạp sau kế hoạch). List Xuất kho có 3 cột **Dòng xe con · Tải (n/cap · %) · Cước dự tính**, % tải tính SỐNG từ `loadOf` + `loadUtilization` (freight.ts, test), Non tải đỏ khi dưới `underload_pct`; detail hiện Xe · Tải · Cước. Gói 60 [6a–6h2] oracle độc lập (giá × ceil(pallet từ master)).
> **Chưa làm:** báo cáo Non tải theo kho/ĐVVT/dòng xe (mục 15 phần báo cáo) · ô band Σ cước trên list (cần RPC summary) · cột cước ở tab Kế hoạch VC (TMS) · oracle hai đường loadCalc ↔ `gdo_weight_estimates`. Dữ liệu: staging có 5.526 dòng cước SƠ BỘ ước tính (note 'SƠ BỘ…'), bảng cước thật chờ user; 60 dòng con đã gán cha tự động, user rà lại.
