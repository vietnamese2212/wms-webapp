...
| C13 | **Một CỘT hai NGHĨA, mỗi cửa hiểu một kiểu** — `qa_status_id` có mã `OK` = ĐÃ DUYỆT, nhưng cửa quét hiểu đúng còn 5 chỗ chỉ-đường + 3 RPC coi "có giá trị = đang giữ" ⇒ quét thì xuất được mà kế hoạch bảo "hết hàng" | (3) ratchet `qa_hold_rule_hand_rolled` baseline 0 + (2) `tests/unit/qaHold.test.ts` + (1) `isPickEligible` BẮT BUỘC tham số bộ id giữ | 13/09 diễn tập: 8.760 pallet "kẹt" (thật 5) · 83 mã không chốt nổi %Date | 13/09 |
