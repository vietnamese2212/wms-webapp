...
function DailyTab({ canCreate, perms }: { canCreate: boolean; perms: ModulePermissions | null }) {
  ...
  if (sel) return <div className="flex-1 min-h-0 flex flex-col"><SheetPanel sheetId={sel} warehouses={warehouses as { id: string; name: string }[]} perms={perms} onBack={() => setSel(null)} /></div>
        ...
        {step === 'demand' ? (
          ...
          : (
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-slate-100">
                {empRows.map((g, i) => {
                  const rowCls = g.leave ? 'text-red-600' : !g.positions.length ? 'text-slate-400' : 'text-slate-700'
                  return (
                        ...
                        {g.leave ? <span className="italic whitespace-nowrap">Nghỉ phép</span>
                          : canEdit && !locked ? (
                            <div className="flex flex-wrap items-center gap-1">
                              {g.positions.map(s => (
                                <span key={s} className="inline-flex items-center gap-1 bg-sky-50 text-sky-700 border border-sky-200 rounded px-1.5 py-0.5 whitespace-nowrap">
                                  {labelOf(s)}
